...
Once the `conda-forge` channel has been enabled, `pre-commit-mirror-maker` can be installed with:

```
conda install pre-commit-mirror-maker
```

It is possible to list all of the versions of `pre-commit-mirror-maker` available on your platform with:
